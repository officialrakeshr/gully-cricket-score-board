...
const ScoreBoard = () => {
  ...
  const maxWicketCount = parseInt(data.teamSize) - 1;
  ...
  const addWideEffect = () => {
    const scoreTypesButtons = document.querySelectorAll(".score-types-button");
    for (let i = 0; i < scoreTypesButtons.length; i++) {
      scoreTypesButtons[i].classList.add("score-types-button-wide");
    }
    setWide(true);
    setNoBall(false);
  };
  const removeWideEffect = () => {
    const scoreTypesButtons = document.querySelectorAll(".score-types-button");
    for (let i = 0; i < scoreTypesButtons.length; i++) {
      scoreTypesButtons[i].classList.remove("score-types-button-wide");
    }
    setWide(false);
    setNoBall(false);
  };
  ...
  const handleWicket = (isRunOut, playerId) => {
    ...
    if (isNoBall || isWide) {
      if (isRunOut && wicketCount + 1 === maxWicketCount) {
        const endInningButton = document.getElementById("end-inning");
        endInningButton.disabled = false;
        const bowlerNameElement = document.querySelector(
          ".react-autosuggest__input"
      ...
      if (wicketCount + 1 === maxWicketCount) {
        const endInningButton = document.getElementById("end-inning");
        endInningButton.disabled = false;
        const bowlerNameElement = document.querySelector(
          ".react-autosuggest__input"
  ...
  const handleOutTypeChange = (e) => {
    ...
    if (outTypeValue === RUN_OUT) {
      ...
      const runOutPlayerRunElement = document.getElementById(
        "run-out-player-runs"
      );
      runOutPlayerRunElement.classList.remove("hide");
  ...
  const handleRunOutPlayerRuns = (e) => {
    setRunOutRuns(parseInt(e.target.value));
  };
  const endMatch = () => {
    disableAllScoreButtons();
    const endInningButton = document.getElementById("end-inning");
  ...
  if (inningNo === 2) {
    ...
    if (
      ...
      winningMessage = `${chessingTeam} won by ${
        maxWicketCount - wicketCount
      } wickets`;
      endMatch();
    }
    if (
      (wicketCount >= maxWicketCount || overCount >= maxOver) &&
      totalRuns < target - 1
    ) {
  ...
  const firstInningCompletedContent = (
    <>
      ...
      {wicketCount === maxWicketCount && <div>All Out</div>}
    ...
    <div className="container">
      ...
      <div id="badge" className="badge badge-flex">
        {inningNo === 2
          ...
          : overCount === maxOver || wicketCount === maxWicketCount
      ...
      <div className="score-container">
            ...
            <Box sx={radioGroupBoxstyle}>
              <FormControl component="fieldset">
                ...
                >
                  ...
                  <select
                    defaultValue="0"
                    id="run-out-player-runs"
                    className="run-out-player hide"
                    onChange={handleRunOutPlayerRuns}
                  >
                    <option value="" disabled>
                      select runs
                    </option>
                ...
                >
                  Please select run out player name & runs
                </div>
              </FormControl>
            </Box>
          </Modal>
        ...
        <div className="score">
          <div>
            {inningNo === 1 ? scoringTeam : chessingTeam} : {totalRuns}/
            {wicketCount} ({totalOvers})
          </div>
          ...
          <table>
            ...
            <tbody>
              <tr>
                ...
                <td className="score-types">
                  {batter1.run === undefined
                    ? `0(0)`
                    : `${batter1.run}(${batter1.ball})`}
                </td>
                <td className="score-types">
                ...
                <td className="score-types">
                  {batter1.strikeRate === undefined ? 0 : batter1.strikeRate}
                </td>
              </tr>
              <tr>
                <td className="score-types">
                ...
                <td className="score-types">
                  {batter2.run === undefined
                    ? `0(0)`
                    : `${batter2.run}(${batter2.ball})`}
                </td>
                <td className="score-types">
                ...
                <td className="score-types">
                  {batter2.strikeRate === undefined ? 0 : batter2.strikeRate}
                </td>
              </tr>
            </tbody>
          </table>
              ...
              <table>
                ...
                <tbody>
                  ...
                  <tr>
                    ...
                    <td className="score-types">
                      {inningNo === 1 ? extras.total : inning1.extra.total}
                    </td>
                    <td className="score-types text-center">
                      wd:{inningNo === 1 ? extras.wide : inning1.extra.wide}
                    </td>
                    <td className="score-types text-center">
                      nb:{inningNo === 1 ? extras.noBall : inning1.extra.noBall}
                    </td>
                ...
                <table>
                  ...
                  <tbody>
                    ...
                    <tr>
                      <td className="score-types padding-left">Extras:</td>
                      <td className="score-types">
                        {hasMatchEnded ? inning2.extra.total : extras.total}
                      </td>
                      <td className="score-types text-center">
                        wd:{hasMatchEnded ? inning2.extra.wide : extras.wide}
                      </td>
                      <td className="score-types text-center">
                        nb:
                        {hasMatchEnded ? inning2.extra.noBall : extras.noBall}
                      </td>
                      <td className="score-types text-center"></td>
                    </tr>
                  </tbody>
